const peliculas = [
  ...
  },
  {
    ...
    anio: 1998,
    imagen: "https://images.justwatch.com/poster/76802182/s718/el-gran-lebowski.jpg",
...
const contenedor = document.getElementById("listado-peliculas");
const buscador = document.getElementById("buscador");
const filtroFecha = document.getElementById("filtro-fecha");
const paginacionContainer = document.getElementById("paginacion");


const peliculasPorPagina = 9; 
let paginaActual = 1;
let peliculasFiltradas = [...peliculas]; 

function mostrarPeliculas(lista, pagina = 1) {
  ...
  if (peliculasPagina.length === 0) {
    contenedor.innerHTML = `
      <div class="col-12 text-center py-5">
        <h3>No se encontraron películas con ese criterio</h3>
      </div>
    `;
      ...
      <div class="col-12 col-sm-6 col-md-4 mb-4">
        <div class="card h-100">
          ...
          <div class="card-body d-flex flex-column">
            ...
            <p class="card-text text-muted">${p.anio}</p>
            <p class="card-text">${p.sinopsis.substring(0, 80)}...</p>
            <a href="detalle.html?id=${p.id}" class="btn btn-primary mt-auto">Ver más</a>
          </div>
        </div>
      </div>
    `;
...
function actualizarPaginacion(lista) {
  const totalPaginas = Math.ceil(lista.length / peliculasPorPagina);
  paginacionContainer.innerHTML = "";
  ...
  let paginacionHTML = `
    <nav aria-label="Navegación de películas">
  ...
  let startPage = Math.max(1, paginaActual - Math.floor(mostrarPaginas / 2));
  let endPage = Math.min(totalPaginas, startPage + mostrarPaginas - 1);
  
 
  if (endPage - startPage + 1 < mostrarPaginas) {
    startPage = Math.max(1, endPage - mostrarPaginas + 1);
  }
...
filtroFecha.addEventListener("change", filtrarPeliculas);


mostrarPeliculas(peliculas, paginaActual);